"""
Exercise 4: Tree of Thought — Explore Multiple Reasoning Branches
Goal: Generate N candidate answers, evaluate each, continue only the best branch.

Pattern:
  Level 0: Original question
  Level 1: Generate 3 candidate approaches/answers
  Level 2: Evaluate each → score 1-10
  Level 3: Expand the best 1-2 candidates (go deeper)
  Final:   Pick the best leaf node

Tasks:
  1. Complete generate_thoughts() — given a question + context, return N candidate thoughts.
  2. Complete evaluate_thought() — score a thought 1-10 given the original question.
  3. Complete expand_thought() — deepen a thought into a fuller answer.
  4. Complete tree_of_thought() — run 2 levels of exploration, return best final answer.
  5. Compare ToT vs single-shot on the test question — does ToT score higher?

Expected output:
  Level 1: 3 thoughts generated
    T1 (score 6): "Focus on economic factors..."
    T2 (score 8): "Use the STAR method..."  ← best
    T3 (score 5): "List pros and cons..."
  Level 2: expanding T2...
  Final answer (score 8): ...
"""
import os
import sys
...
def generate_thoughts(question: str, context: str = "", n: int = 3) -> list[str]:
    """Generate n distinct candidate approaches."""
    user_msg = f"Question: {question}"
    if context:
        user_msg += f"\n\nBuild on this approach: {context}"
    system = GENERATOR_SYSTEM.format(n=n)
    # TODO: call chat() with system and user_msg
    # TODO: raw = get_text(response)
    # TODO: parse JSON array from raw
    # TODO: return list of strings
    raise NotImplementedError


def evaluate_thought(question: str, thought: str) -> tuple[float, str]:
    """Score a thought 1-10. Return (score, reason)."""
    prompt = f"Question: {question}\n\nApproach/Answer:\n{thought}"
    # TODO: call chat() with EVALUATOR_SYSTEM and prompt
    # TODO: parse JSON, return (data["score"], data["reason"])
    raise NotImplementedError


def expand_thought(question: str, thought: str) -> str:
    """Develop a thought into a full answer."""
    prompt = f"Question: {question}\n\nInitial approach: {thought}\n\nDevelop this into a full answer."
    # TODO: call chat() with EXPANDER_SYSTEM and prompt
...
def tree_of_thought(question: str, branching: int = 3, depth: int = 2,
    ...
    branching: candidates at each level
    depth: how many levels to expand
    keep_top: how many best candidates to expand at each level
    Returns (best_final_answer, all_thoughts)
    """
    print(f"\nQuestion: {question}")
...
def compare_approaches(question: str):
    """Compare single-shot vs Tree of Thought on the same question."""
    print("="*60)
    print("SINGLE-SHOT:")
